...
class HomeAPI: WebService {
    ...
    func getseasons(page:Int,completion:@escaping (SeasonsResponse?) -> Void, failed: @escaping (String) -> Void){
        let url = "https://app.8com.cloud/api/v1/sportscore/data/season.php"
        ...
        post(url: url, params: param, completion: { json in
            let response = SeasonsResponse(json!)
            completion(response)
        }, failed: failed)
    }
    
    func getseasonDetails(slug:String,completion:@escaping (SeasonDetailsResponse?) -> Void, failed: @escaping (String) -> Void){
        let url = "https://app.8com.cloud/api/v1/sportscore/data/league.php"
        var lang = Utility.getCurrentLang()
        if lang == "cn"{
            lang = "zh"
        }
        let param:[String:Any] = ["lang":lang,
                                  ...
                                  "season":""]
        post(url: url, params: param, completion: { json in
            let response = SeasonDetailsResponse(json!)
            completion(response)
        }, failed: failed)
    }
    
    
    func getPlyearsList(pageNumber:Int,completion:@escaping (PlayerDetailsResponse?) -> Void, failed: @escaping (String) -> Void){
        ...
        post(url: url, params: param, completion: { json in
            let response = PlayerDetailsResponse(json!)
            completion(response)
        }, failed: failed)
    }
